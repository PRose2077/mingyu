export const PROMPT_GUIDANCE_TEXT = {
  ...
  liuyao: {
    tradition:
      '六爻先定用神和世应，再结合月建、日辰旺衰、动爻变爻、空破、伏神及原忌仇神作用链判断。',
    sources: '参考京房八宫纳甲体系及《火珠林》《卜筮正宗》《增删卜易》等资料。',
  },
  meihua: {
    tradition: '以本卦、互卦、变卦和动爻为结构，结合体用生克、四时旺衰与卦爻辞取象。',
  ...
  jinkoujue: {
    tradition:
      '金口诀以地分、将神、贵神、人元四位为主，结合阴阳发用、月令旺衰、五位生克与五动三动判断。',
  ...
  liuren: {
    tradition:
      '大六壬以月将加时、四课和取传规则定发用；三传看发端、转折与归结，并结合旺衰、空亡、天将、课体和类神判断。',
...
export const PROMPT_ANSWER_FRAMEWORK =
  '先给出明确的倾向或吉凶定性，再说明主要理据及其生克演变；涉及阶段变化时，依据已列干支时段、节令或时间层级说明。';

/** 各术数体系专属的传统推演骨架，针对不同底层象数理模型定制，保障沉浸与地道。 */
export const PROMPT_METHOD_ANSWER_FRAMEWORKS: Record<string, string> = {
  // 1. 命理时序体系
  bazi: '先立足四柱原局判定旺衰调候、格局用神与性格根基，明确主干吉凶定性；再结合岁运干支引动推演阶段吉凶与流年应事，时间判断须落实到具体干支时段或节令。',
  'bazi-natal':
    '先立足四柱原局判定旺衰调候、格局用神与性格根基，再依据盘面所列月令、根气、透干与制化说明结构倾向。',
  'bazi-compatibility':
    '先分别确立双方原局旺衰调候与性格基调，再交叉比较日主五行、十神配合与合冲刑害。',
  ziwei:
    '先以命身十二宫与三方四正定本命格局，再结合生年四化与运限落宫飞星，详述阶段起伏与吉凶应期。',
  'ziwei-natal': '先以命身十二宫与三方四正定本命格局，再结合生年四化说明结构、倾向与宫位关系。',
  'ziwei-compatibility': '先分别确立双方命身与关系宫位格局，再交叉比较跨盘星曜与四化落点。',
  'bazi-ziwei': '八字先看原局，紫微先看命身四化，两套体系各自成论后交叉印证。',
  'bazi-ziwei-aligned': '八字按所列岁运、紫微按所列运限各自成论，再交叉印证同一时间范围。',
  'bazi-ziwei-mismatch': '八字与紫微按各自已列资料成论后交叉印证，时间层未对齐时分开陈述。',
  astrolabe:
    '先以太阳、月亮、上升与主要相位格局定本命底色；有行运或周期星象资料时，推演必须结合具体星象周期落地时间窗口。',
  'astrolabe-natal': '先以太阳、月亮、上升与主要相位格局定本命底色与结构关系。',
  'astrolabe-synastry':
    '先分别确立双方本命结构与关系需求，再结合跨盘相位与重要落宫推演互动张力与长期承接。',
  qizheng: '先以命身宫度与十一星落宿定本命根基，再依吊照详析；有行限或流曜周期时再看阶段起伏。',

  // 2. 卦象筮法与三式体系
  liuyao:
    '先辨世应用神旺衰与月破暗动决断吉凶成败，再析动变伏藏与进退生克，应期推演必须结合动逢值冲、空逢冲合等具体干支时序。',
  meihua:
    '先以体用旺衰与生克实效决断吉凶成败，再详析互卦过程与变卦归宿，并依据动爻卦数指明应期时序。',
  'meihua-number':
  ...
  liuren:
    '先以初传发用定事之发端，中传观移易转折，末传看归宿终局；结合天将乘克与课体格局详断吉凶演变。',
  jinkoujue: '先以四位生克与阴阳发用定吉凶成败，再结合五动三动与神将旺衰详析事态结构与相对节奏。',
  ...
  // 4. 择日体系
  almanac:
    '比较全部日期，给出首选、备选与慎用日期，并说明事项宜忌、建除、宿曜、冲煞和参与人年命的关键取舍；仅在设置时段条件时复核备选时辰。',
